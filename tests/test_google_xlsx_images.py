...
def sample_export(image_column=11):
    ...
    files = {
        ...
        "xl/drawings/drawing1.xml": f'<xdr:wsDr xmlns:xdr="{drawing}" xmlns:a="{drawing_main}" xmlns:r="{rel}"><xdr:oneCellAnchor><xdr:from><xdr:col>{image_column}</xdr:col><xdr:row>1</xdr:row></xdr:from><xdr:pic><xdr:blipFill><a:blip r:embed="rId1"/></xdr:blipFill></xdr:pic></xdr:oneCellAnchor></xdr:wsDr>',
...
class ImageExportTests(unittest.TestCase):
    def test_compact_layout_reads_shifted_image_column(self):
        self.assertEqual(parse_export(sample_export(image_column=9), "Анатомия",
                                      [["ID карточки"], ["card_1"]],
                                      columns=(10, 11), text_columns=9), {(2, 10): PNG})
